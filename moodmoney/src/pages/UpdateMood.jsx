...
export default function UpdateMood() {
  ...
  useEffect(() => {
    const fetchExpense = async () => {
      try {
        ...
        const found = res.data.expenses.find((exp) => exp._id === expenseId)
        if (!found) {
          setError("Expense not found")
        } else {
          setOriginalExpense(found)
  ...
  const handleSave = async () => {
    try {
      ...
      await axios.put(`http://localhost:5050/api/expenses/${userId}/${expenseId}`, {
        mood: selectedMood,
      })
  ...
  return (
    <div className="w-screen min-h-screen bg-white px-6 py-6 flex flex-col items-center">
      <h1 className="text-3xl font-luckiest text-green-600 mb-4 text-center">UPDATE YOUR MOOD</h1>
      ...
      {originalExpense && (
        <div className="w-full mb-6">
          <h2 className="text-2xl font-luckiest text-green-600 mb-2 text-center">ORIGINAL PURCHASE</h2>

          <div className="text-center mb-4">
            <p className="text-lg">
              <span className="text-green-600 font-bold font-luckiest">AMOUNT SPENT: </span>
              <span className="text-black font-medium font-luckiest">${originalExpense.amount}</span>
            </p>
            <p className="text-md">
              <span className="text-green-600 font-bold font-luckiest">CATEGORY: </span>
              <span className="text-black font-medium font-luckiest">{originalExpense.category}</span>
            </p>
            <p className="text-md">
              <span className="text-green-600 font-bold font-luckiest">PURCHASE DATE: </span>
              <span className="text-black font-medium font-luckiest">{formatDate(originalExpense.date)}</span>
            </p>
            <p className="text-md">
              <span className="text-green-600 font-bold font-luckiest">MOOD SELECTED: </span>
              <span className="text-black font-medium font-luckiest">{originalExpense.mood || "None"}</span>
            </p>
          </div>
        </div>
      )}
